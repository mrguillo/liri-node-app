...
switch (liriArgument) {
	...
	default:
		console.log("=========== MY NAME IS LIRI, CHOOSE ONE OF OPTION COMMANDS BELOW: ==============\n" +
...
// =================================================================
// concert-this <artist/band name here> function with BANDS IN TOWN + AXIOS packages
	...
	axios.get("https://rest.bandsintown.com/artists/" + artist + "/events?app_id=codingbootcamp")
		.then(function (response) {
			...
			response.data.forEach(concert => {
				...
				var concerts = space + "---------------------------------------------------" +
				space + concert.venue.name +
				space + concert.venue.city + ", " + (concert.venue.region || concert.venue.country) +
				space + (moment(concert.datetime).format("MM/DD/YYYY"));

				// In addition to logging the data to terminal/bash window, output the data to `log.txt`
				fs.appendFile("log.txt", concerts, function (err) {
					if (err) throw err;
				})
			})
			console.log(space);
			console.log('Liri saved this answer to the log.txt file for you!');
...
// =================================================================
// Spotify function, Node Spotify API package
	...
	if (songName === undefined) {
		...
		spotify.search({
			...
			fs.appendFile("log.txt", output, function (err) {
				if (err) throw err;
				console.log('This Liri answer was saved to the log.txt file!');
			})
		});
	} else {
		spotify.search({
...
//=================================================================
// Movie-this function, OMDB api with AXIOS package
	...
	axios.get(queryUrl).then(
			function (response) {
				...
				var movieResults = " ===================== LIRI PROVIDED THIS DATA FOR YOU...====================\n" +
					...
					space + "Rotten Tomatoes Rating: " + response.data.Ratings[1].Value +
